...
title KS Preupdate Restore 1682043

set "SOURCE=C:\KristalStreams168RC1R2\KristalStreams-1.6.8-RC1-R2-LEGACY-DEMO-FIX"
for /f %%T in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd-HHmmss"') do set "STAMP=%%T"
set "WORK=C:\ksseriespreupdaterestore-!STAMP!"
set "FINAL=%USERPROFILE%\Downloads\KS-PREUPDATE-RESTORE-1682043.apk"
set "LOG=%TEMP%\ks-preupdate-restore-1682043-build.txt"
set "JAVASAVE=%USERPROFILE%\.kristalstreams-java-home.txt"

echo.
echo ==========================================================
echo   KRISTAL STREAMS 1.6.8 RC1 R2 - PREUPDATE RESTORE
echo   FRESH APK: KS-PREUPDATE-RESTORE-1682043.apk
echo ==========================================================
echo.
echo Baseline: known-good R2
echo Restores the last successfully installed pre-1682042 Series build.
...
echo Keeps the Season selector above the compact Continue and Next panel.
...
if errorlevel 1 (
    echo ERROR: Could not extract the landscape episode-lock files.
    pause
    exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%EPISODELOCKPS%" -ProjectRoot "%WORK%"
set "RC=%ERRORLEVEL%"
del /q "%EPISODELOCKPS%" >nul 2>&1
if not "%RC%"=="0" (
    echo ERROR: The landscape episode-lock correction could not be applied safely.
    pause
    exit /b %RC%
)

echo [12/16] Matching landscape Resume and Next controls to the surrounding cards...
set "OVERLAPPS=%TEMP%\ks-preupdate-restore-1682043.ps1"
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
 "$p='%WORK%\app\build.gradle.kts'; $c=[IO.File]::ReadAllText($p); if(-not $c.Contains('versionCode = 1682021')){throw 'Expected 1682021 version code was not found'}; $c=$c.Replace('versionCode = 1682021','versionCode = 1682043').Replace('1.6.8-series-complete','1.6.8-preupdate-restore'); [IO.File]::WriteAllText($p,$c,[Text.UTF8Encoding]::new($false))"
if errorlevel 1 (
    echo ERROR: Could not set the new 1682043 application version.
    pause
    exit /b 1
)
...
findstr /c:"versionCode = 1682043" "%WORK%\app\build.gradle.kts" >nul
if errorlevel 1 (
    echo ERROR: New 1682043 application version verification failed.
    pause
    exit /b 1
)
...
findstr /c:"android:elevation=\"12dp\"" "%WORK%\app\src\main\res\layout-land\activity_series_details.xml" >nul
if errorlevel 1 (
    echo ERROR: Fixed episode-pane foreground verification failed.
    pause
    exit /b 1
)
findstr /c:"android:id=\"@+id/seriesEpisodePane\"" "%WORK%\app\src\main\res\layout-land\activity_series_details.xml" >nul
if errorlevel 1 (
    echo ERROR: Landscape episode-pane ID verification failed.
    pause
    exit /b 1
)
...
findstr /c:"Choose an episode" "%WORK%\app\src\main\java\com\kristalstreams\player\SeriesDetailsActivity.kt" >nul
if errorlevel 1 (
    echo ERROR: Short Choose Episode prompt verification failed.
    pause
    exit /b 1
)
...
findstr /c:"android:id=\"@+id/seriesContinuePanel\" android:layout_width=\"match_parent\" android:layout_height=\"110dp\" android:clipChildren=\"true\" android:clipToPadding=\"true\"" "%WORK%\app\src\main\res\layout\activity_series_details.xml" >nul
if errorlevel 1 (
    echo ERROR: Portrait Choose Episode boundary verification failed.
    pause
    exit /b 1
)
findstr /c:"android:visibility=\"invisible\"" "%WORK%\app\src\main\res\layout-land\activity_series_details.xml" >nul
if errorlevel 1 (
    echo ERROR: Stable landscape control-space verification failed.
    pause
    exit /b 1
)
findstr /c:"android:clipToPadding=\"true\"" "%WORK%\app\src\main\res\layout-land\activity_series_details.xml" >nul
if errorlevel 1 (
    echo ERROR: Landscape episode clipping verification failed.
    pause
    exit /b 1
)
findstr /c:"private fun isMobileLandscape()" "%WORK%\app\src\main\java\com\kristalstreams\player\SeriesDetailsActivity.kt" >nul
if errorlevel 1 (
    echo ERROR: Mobile landscape detection verification failed.
    pause
    exit /b 1
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
 "$p='%WORK%\app\src\main\res\layout-land\activity_series_details.xml'; $x=[IO.File]::ReadAllText($p); if($x.IndexOf('seriesContinuePanel') -gt $x.IndexOf('episodeList')){throw 'Episodes are not below the compact Continue panel'}"
if errorlevel 1 (
    echo ERROR: Landscape episode-boundary ordering verification failed.
    pause
    exit /b 1
)
...
if defined USBDRIVE (
    set "USBCOPY=!USBDRIVE!\KS-PREUPDATE-RESTORE-1682043.apk"
...
echo.
echo Install only KS-PREUPDATE-RESTORE-1682043.apk shown above.
...
:::BEGIN EPISODELOCKPATCH

...

