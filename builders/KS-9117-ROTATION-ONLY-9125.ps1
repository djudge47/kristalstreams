...
$work = "C:\ks-9117-rotation-only-9125-$stamp"
$decoded = Join-Path $work 'decoded'
$unsigned = Join-Path $work 'KristalStreams-KS-ROTATION-ONLY-9125-unsigned.apk'
$aligned = Join-Path $work 'KristalStreams-KS-ROTATION-ONLY-9125-aligned.apk'
$desktop = [Environment]::GetFolderPath('Desktop')
$final = Join-Path $desktop 'KristalStreams-KS-ROTATION-ONLY-9125.apk'

Write-Host ''
Write-Host 'KRISTAL STREAMS 9117 - ROTATION ONLY' -ForegroundColor Cyan
Write-Host 'BASE: existing 9117 labeled-refresh APK' -ForegroundColor Green
...
# Find the real 9117 build wherever the earlier KS work placed it, without scanning the whole C: drive.
...
foreach ($root in $searchRoots) {
    Get-ChildItem -LiteralPath $root -File -Filter '*9117*.apk' -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
        [void]$candidates.Add($_)
    }
}
Get-ChildItem -LiteralPath 'C:\' -File -Filter '*9117*.apk' -ErrorAction SilentlyContinue | ForEach-Object {
    [void]$candidates.Add($_)
}
...
foreach ($apk in ($candidates | Sort-Object FullName -Unique)) {
    ...
    $certLine = @(& $apksigner verify --print-certs $apk.FullName 2>$null) | Select-String 'certificate SHA-256 digest:' | Select-Object -First 1
    if (!$certLine) { continue }

    $priority = 3
    if ($apk.Name -eq 'KristalStreams-KS-LABELED-REFRESH-9117-TEST.apk') { $priority = 0 }
    elseif ($apk.Name -match 'LABELED-REFRESH-9117.*TEST') { $priority = 1 }
    elseif ($apk.Name -match '9117') { $priority = 2 }

    [void]$valid.Add([pscustomobject]@{
        File = $apk
        Priority = $priority
        Cert = ($certLine.Line -split ':\s*',2)[1].Trim().ToLowerInvariant()
    })
}

$selected = $valid | Sort-Object Priority, @{Expression={$_.File.LastWriteTime};Descending=$true} | Select-Object -First 1
if (!$selected) {
    throw 'Signed Kristal Streams 9117 APK was not found in the known KS work locations. Nothing was changed.'
}
...
Write-Host '[1/4] Decoding 9117...' -ForegroundColor Cyan
...
# Install version bump only. Keep original versionName and every other field unchanged.
...
Write-Host '[2/4] Rebuilding with rotation unlocked...' -ForegroundColor Cyan
...
if (!$finalCert) { throw 'Could not verify finished signing certificate.' }
...
Write-Host 'BASE: 9117 LABELED-REFRESH APP' -ForegroundColor Green
