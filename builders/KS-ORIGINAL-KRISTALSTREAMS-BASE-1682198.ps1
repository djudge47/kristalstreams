...
Write-Host 'ROTATION / LAYOUTS: ORIGINAL APK PRESERVED - NO DONOR APP' -ForegroundColor Green
Write-Host ''

if (!(Test-Path -LiteralPath $ksSourceApk)) {
    throw "Original APK not found: $ksSourceApk"
}

$ksBuildToolsRoot = Join-Path $env:LOCALAPPDATA 'Android\Sdk\build-tools'
if (!(Test-Path -LiteralPath $ksBuildToolsRoot)) {
    throw "Android SDK build-tools not found: $ksBuildToolsRoot"
}
...
if ($ksSourcePackage -ne $ksExpectedPackage) {
    throw "STOP: wrong APK. Expected $ksExpectedPackage but found $ksSourcePackage"
}
if ($ksSourceVersion -ne $ksExpectedSourceVersion) {
    throw "STOP: this is not the original 1681003 kristalstreams.apk. Found versionCode $ksSourceVersion"
}
...
if ($LASTEXITCODE -ne 0 -or !(Test-Path -LiteralPath (Join-Path $ksDecoded 'AndroidManifest.xml'))) {
    throw 'Original kristalstreams.apk decode failed'
}
...
$ksYamlText = [regex]::Replace($ksYamlText, '(?m)^(\s*versionCode:\s*).+$', ('$1' + $ksTargetVersion), 1)
if ($ksYamlText -match '(?m)^\s*versionName:\s*') {
    $ksYamlText = [regex]::Replace($ksYamlText, '(?m)^(\s*versionName:\s*).+$', ('$1' + $ksTargetVersionName), 1)
}
...
if ($ksManifestText -notmatch 'package="com\.kristalstreams\.player"') {
    throw 'STOP: decoded package identity is not com.kristalstreams.player'
}

# Important: do not change screenOrientation, activity routing, resource qualifiers,
# provider/login code, EPG, playback, or dashboard geometry. The original APK's
# native portrait/landscape behavior stays intact.

Write-Host '[2/4] Building from the original engine with native rotation untouched...' -ForegroundColor Cyan
...
Write-Host '[4/4] Verifying package and version...' -ForegroundColor Cyan
...
$ksFinalSize = (Get-Item -LiteralPath $ksFinal).Length
if ($ksFinalSize -lt 8MB) { throw 'Finished APK is unexpectedly small - STOP' }
...
Write-Host 'NATIVE ORIGINAL ROTATION / LAYOUTS: PRESERVED' -ForegroundColor Green
