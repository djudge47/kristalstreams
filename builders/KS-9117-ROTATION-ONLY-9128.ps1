...
$work = "C:\ks-9117-rotation-only-9128-$stamp"
$decoded = Join-Path $work 'decoded'
$unsigned = Join-Path $work 'KristalStreams-KS-ROTATION-ONLY-9128-unsigned.apk'
$aligned = Join-Path $work 'KristalStreams-KS-ROTATION-ONLY-9128-aligned.apk'
...
# Apktool's VersionInfo parser expects versionCode as an integer, not a quoted YAML string.
...
if (-not (Select-String -LiteralPath $yamlPath -Pattern ('^\s*versionCode:\s*' + $newVersion + '\s*$') -Quiet)) {
    throw 'Numeric versionCode verification failed before build.'
}
...
if ($finalVer -ne [string]$newVersion) { throw "Finished APK versionCode is $finalVer instead of $newVersion" }
